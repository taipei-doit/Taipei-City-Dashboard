"""Test for fire_hospital_capacity DAG.

驗證 data_infos.source 可達且回傳合理資料。**不**需要 Airflow / Postgres。

Run from DAG folder:
    python test_fire_hospital_capacity.py
或從 toolkit:
...
RID = ""   # 此 example 是 CSV,不需要 RID
# csv / csv-big5 編碼:utf-8 / big5 / cp950
ENCODING = "big5"   # source_type=csv 時生效;此 example 為 csv-big5(透過 source_type 自動帶 big5)
