"""Test for purchase_subsidy_application_status DAG.

驗證 data.ntpc 來源可達且回傳必要欄位。**不**需要 Airflow / Postgres。

Run from DAG folder:
    python test_purchase_subsidy_application_status.py
或從 toolkit:
...
def test_source_url_reachable():
    ...
    print(f"  data.ntpc reachable, keys: {list(ntpc_records[0].keys())[:10]}")
