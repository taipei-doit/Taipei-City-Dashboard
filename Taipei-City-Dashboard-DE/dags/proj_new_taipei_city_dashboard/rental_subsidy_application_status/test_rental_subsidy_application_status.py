"""Test for rental_subsidy_application_status DAG.

驗證 data.ntpc 來源可達且回傳必要欄位。**不**需要 Airflow / Postgres。

Run from DAG folder:
    python test_rental_subsidy_application_status.py
或從 toolkit:
...
def test_source_url_reachable():
    ...
    for key in ("year", "rent_apply_num", "rent_project_num", "rent_ok_num"):
        if key not in ntpc_keys:
            raise AssertionError(f"data.ntpc 缺少欄位: {key}")
    print(f"  data.ntpc reachable, keys: {list(ntpc_records[0].keys())[:8]}")
