...
def _park_ntpe(**kwargs):
    ...
    COL_MAP = {
        ...
        "management": 'text COLLATE pg_catalog."default"',
    ...
    data["city"] = "新北市"
    # 範圍門牌(如 30．32號)→ 取第一個號碼(同 green_store)
    data["address"] = data["address"].str.replace(
        r"(\d+)\s*[．。.，,、＆&]\s*\d+(?:之\d+)?號", r"\1號", regex=True
    )
    # 公園地址常見「…號對面/旁/附近/前/後」描述,geocode 前砍掉、保留門牌
    data["address"] = data["address"].str.replace(
        r"號(對面|斜對面|旁邊|旁|附近|前方|前|後方|後).*$", "號", regex=True
    )
    # === Geocode(地址→經緯度;對不到的(如「路口」「與X路交叉」)keep-all-rows 留 null)===
    data["lng"] = None
    data["lat"] = None
...
dag = CommonDag(
    proj_folder="proj_new_taipei_city_dashboard", dag_folder="park_ntpe"
)
dag.create_dag(etl_func=_park_ntpe)
