...
def _family_medicine_institutions_ntpe(**kwargs):
    # === Imports(全部寫在函式內)===
    import pandas as pd
    ...
    raw_data = pd.DataFrame(
        NewTaipeiAPIClient("9e1c1aba-4e0b-4d5a-8755-efa1f09abe65").get_all_data(size=1000)
    )
    # === Transform ===
...
dag = CommonDag(proj_folder="proj_new_taipei_city_dashboard", dag_folder="family_medicine_institutions_ntpe")
dag.create_dag(etl_func=_family_medicine_institutions_ntpe)
