...
def _emergency_medical_institutions_ntpe(**kwargs):
    # === Imports(全部寫在函式內)===
    import pandas as pd
    ...
    raw_data = pd.DataFrame(
        NewTaipeiAPIClient("7e4a9359-61dd-4a3c-9ebc-8e2b0b380a13").get_all_data(size=1000)
    )
    # === Transform ===
...
dag = CommonDag(proj_folder="proj_new_taipei_city_dashboard", dag_folder="emergency_medical_institutions_ntpe")
dag.create_dag(etl_func=_emergency_medical_institutions_ntpe)
