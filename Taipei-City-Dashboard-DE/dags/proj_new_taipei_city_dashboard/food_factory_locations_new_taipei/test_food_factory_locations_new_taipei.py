"""Test for food_factory_locations_new_taipei DAG.

驗證 data_infos.source 可達且回傳合理資料。**不**需要 Airflow / Postgres。

Run from DAG folder:
    python test_food_factory_locations_new_taipei.py
或從 toolkit:
...
def _fetch_data_ntpc(url: str) -> list[dict]:
    ...
    # 新版 data.ntpc.gov.tw OpenAPI（/api/datasets/<rid>/json）直接回傳 list of records
    if isinstance(body, list):
        if not body:
            raise AssertionError("data.ntpc 回應為空陣列")
        return body
    # 舊版 ckan-style fallback
