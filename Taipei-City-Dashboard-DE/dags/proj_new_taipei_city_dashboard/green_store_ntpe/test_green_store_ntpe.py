"""Test for green_store_ntpe DAG.

驗證 data_infos.source 可達且回傳合理資料。**不**需要 Airflow / Postgres。

Run from DAG folder:
    python test_green_store_ntpe.py
或從 toolkit:
...
def _fetch_data_ntpc(url: str) -> list[dict]:
    """data.ntpc 的 /api/datasets/<rid>/json endpoint 直接回 list[dict],
    舊式 CKAN 風格(result.records)則回 dict。兩種 shape 都支援。
    """
    body = _fetch_json(url)
    if isinstance(body, list):
        if not body:
            raise AssertionError("data.ntpc 列表為空")
        return body
    if isinstance(body, dict):
        records = body.get("result", {}).get("records") or body.get("records")
        if records:
            return records
    raise AssertionError(f"data.ntpc 無法解析;body type={type(body).__name__}, sample={str(body)[:200]}")
