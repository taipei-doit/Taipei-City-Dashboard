...
def etl_function(**kwargs):
    ...
    if not all_dfs:
        raise RuntimeError("No Taipei data downloaded")
    ...
    if city_col:
        data = raw_data[raw_data[city_col].str.contains("臺北市", na=False)].copy()
    else:
        data = raw_data.copy()
...
dag = CommonDag(
    ...
    dag_folder="traffic_pedestrian_accident_taipei"
)
