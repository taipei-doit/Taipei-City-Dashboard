...
依序做這 7 大類檢查。每類都用「**雙向比對**」的思考 — 不要單方面列舉，要列出兩邊各自有什麼然後比對 mismatch。

### 0. 評估報告 → DE 規劃 → 實作 三層 spec 追溯（**先做這個**）

**這是最上游的 spec 追溯**。技術層（FE/BE/DB）的問題可能根因在 DE 規劃就已偏離評估報告，所以要先把三層比對做完再看技術層。

**Step 0-1：確立 spec ground truth（評估報告）**

若使用者提供評估報告路徑，讀取其「實作架構與資料接入規劃」段，抽出每個 component 的：
- 編號、名稱、主題描述
- 資料集（dataset ID）
- 視覺化形式（圖表類型）
- 行號（引用用）

若未提供評估報告，在報告的「三層追溯」段標注「未提供評估報告，跳過此節」。

**Step 0-2：讀 DE 規劃的 §11 修正紀錄**

從 DE 規劃文件（若存在）抽出：
- 每個 component 實際規劃的 SQL 主題
- §11 修正紀錄：有沒有標「⚠️ 此修正改變 component 主題」

沒有 §11 登記但主題跟評估報告不同 → **未登記的偷改**。

**Step 0-3：查實作的 API endpoint + FE 呼叫**

- FE 打哪個 endpoint？
- BE 該 endpoint 的回傳主題是什麼？

**Step 0-4：輸出三層比對表（強制三欄）**

```
| 卡片 | 評估報告（行號） | DE 規劃（§X） | 實作（FE/BE） | 偏離方 |
|---|---|---|---|---|
| C1 | COUNT active alerts（L301） | /alert-count ✅ | FE 打 /alert-count ✅ | 對齊 |
| C3 | 異常類型圓餅（L303） | /alert-trend-30d ❌（未登記改主題） | FE 打 /alert-by-type ✅ | DE 規劃偷改（未登記） |
| C4 | 站點底圖 marker（L304） | /stations ✅ | legend cards ❌ | 實作偷改 |
```

**偏離分類**（只能用這五種）：
- `對齊`：三層一致
- `DE 規劃簡化`：DE 省略細節但主題未變，效果等價
- `DE 規劃偷改（未登記）`：主題偏離評估報告，§11 無登記
- `DE 規劃主題變更（已登記）`：主題偏離評估報告，但 §11 有完整說明區塊（合法）
- `實作偷改`：實作偏離 DE 規劃（且 DE 規劃本身對齊評估報告）
- `雙層偷改`：DE 規劃偷改 + 實作又偏離 DE 規劃

**關鍵判斷原則**：找到實作偏離 DE 規劃時，**先反問「DE 規劃本身有沒有偏離評估報告」**，不要直接把 FE/BE 標為錯的——C3 事件就是 DE 規劃偷改，FE 反而是對的。
...
## 📋 三層 Spec 追溯（評估報告 → DE 規劃 → 實作）

<若未提供評估報告，此節寫「未提供評估報告，跳過」>
