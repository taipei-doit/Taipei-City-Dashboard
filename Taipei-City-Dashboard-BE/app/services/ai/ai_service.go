...
func init() {
	...
	if !IsTWCCConfigured() {
		logs.Warn("TWCC API key is not configured; AI chat endpoint will return 503")
	}
}
...
// IsTWCCConfigured reports whether the backend has a usable TWCC API key.
func IsTWCCConfigured() bool {
	apiKey := strings.TrimSpace(global.TWCC.ApiKey)
	return apiKey != "" && apiKey != "default_your_twcc_api_key_here" && apiKey != "your_api_key"
}
