...
-- ── 3-3. component_maps ────────────────────────────────────
-- 【重要】自訂 SVG 只能用在 type = symbol，不能用 circle-color。
--   1) FE：Taipei-City-Dashboard-FE/src/store/mapStore.js → registerInlineSvgIcons()
--      已註冊 id：green-charger、orange-charger、red-charger（與下方 icon-image 字串一致）
--   2) DB：paint 只放 layout（及可選 filter）；勿再寫 circle-*。
--   3) 比例：available / max(total_charging_points,1) ≥0.5 → 綠；≥0.3 → 橘；否則紅。
--
-- 若資料庫仍是舊版 circle，可手動執行：
--   UPDATE public.component_maps SET type = 'symbol', paint = '{
--     "layout": {
--       "icon-image": ["case",
--         [">=", ["/", ["get", "available"], ["max", ["get", "total_charging_points"], 1]], 0.5], "green-charger",
--         [">=", ["/", ["get", "available"], ["max", ["get", "total_charging_points"], 1]], 0.3], "orange-charger",
--         "red-charger"],
--       "icon-size": 1.1,
--       "icon-allow-overlap": true
--     }
--   }'::json WHERE index = 'ev_stations';
--
-- 地圖上以 symbol 顯示充電站；依 available / total_charging_points 比例套用
-- green-charger / orange-charger / red-charger（須與 FE mapStore.registerInlineSvgIcons 註冊 id 一致）
...
VALUES (
    ...
    NULL,
    NULL,
    '{"layout": {"icon-image": ["case", [">=", ["/", ["get", "available"], ["max", ["get", "total_charging_points"], 1]], 0.5], "green-charger", [">=", ["/", ["get", "available"], ["max", ["get", "total_charging_points"], 1]], 0.3], "orange-charger", "red-charger"], "icon-size": 1.1, "icon-allow-overlap": true}}',
    '[{"key":"station_name","name":"充電站名稱"},{"key":"district","name":"行政區"},{"key":"operator_name","name":"營運業者"},{"key":"total_charging_points","name":"充電槍數"},{"key":"connector_types","name":"充電規格"},{"key":"charge_rate","name":"充電費率"},{"key":"parking_rate","name":"停車費率"},{"key":"service_time","name":"服務時間"},{"key":"address","name":"地址"}]'
)
ON CONFLICT (index) DO UPDATE
...
-- DistrictChart 使用 two_d 格式：x_axis = 行政區名, data = 數值
-- 必須包含雙北 41 個行政區，缺少資料的行政區以 0 填充
...
VALUES (
  ...
  '交通部 TDX 運輸資料流通服務',
  '臺北市各行政區電動車充電站充電槍數分布',
  '呈現雙北41個行政區的電動車充電站充電槍總數（total_charging_points）。資料來源為交通部TDX平台，涵蓋各類充電規格（AC/DC）與多家營運商。點擊地圖站點可查看詳細費率與服務資訊。',
