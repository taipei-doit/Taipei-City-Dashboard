...
function parseLinks(raw) {
  ...
  return rows
    ...
    .filter(
      (it) =>
        it.source && it.target && Number.isFinite(it.value) && it.value > 0,
    );
}
...
const sunburstData = computed(() => {
  ...
  const total = Math.max(
    1,
    outsum.get(root) || level1.reduce((a, b) => a + b.value, 0),
  );
  const cx = W / 2;
  const cy = H / 2;
  ...
  function recurse(
    ...
    const pSum =
      outsum.get(parentName) || kids.reduce((s, x) => s + x.value, 0);
    let cur = a0;
    for (const k of kids) {
...
.sunburstchart__label {
  font-family:
    "微軟正黑體", "Microsoft JhengHei", "Droid Sans", "Open Sans", "Helvetica",
    sans-serif;
  font-size: 14px;
  font-weight: 700;
  fill: #e6edf5;
