---
title: "文件｜臺北城市儀表板"
source: "https://test-citydashboard.taipei/documentation/front-end/custom-maps"
author:
published:
created: 2026-04-24
...
地圖初始化後不久， `mapStore` 的 `initializeBasicLayers` 函式會被呼叫，並在底圖添加了三個預設圖層。其中兩個添加的圖層為臺北市的行政區和里界標籤，並以本地 `geojson` 儲存；而第三個則為臺北市建築物的 3d 模型，以 Mapbox Tiles 形式儲存，此圖層相關設定請參閱 [此先前文章](https://test-citydashboard.taipei/documentation/front-end/project-setup) 。這三個圖層的樣式也儲存在 `mapConfig.js` 文件中。
