...
source: "https://test-citydashboard.taipei/documentation/data-end/pipeline"
author:
published:
created: 2026-04-24
...
如 [撰寫Python程式碼](https://test-citydashboard.taipei/documentation/data-end/dag-code) 章節所示，本專案使用自製的 pipeline 來隱藏程式與 Airflow 互動的細節。本章將說明我們所使用的 `common_pipeline` 的設計理念與程式。
...
由於實際程式碼篇幅較長，請至 `dags\operators\common_pipeline.py` 查看原始碼，或查看 [GitHub連結](https://test-citydashboard.taipei/documentation) 。為了方便理解，說明將從程式碼下方往上閱讀。
...
looks\_two task `etl` 代入各種設定，實際執行資料處理。此步驟所執行的內容並不在 `common_pipeline.py` ，而是 [撰寫Python程式碼](https://test-citydashboard.taipei/documentation/data-end/dag-code) 章節所示範的，一般情況所需開發的每個 DAG 獨立的 ETL 程式碼。
...
[auto\_fix\_highEdit this Page on Github](https://github.com/taipei-doit/Taipei-City-Dashboard-Documentation/edit/main/src/assets/articles/data-end-en/pipeline.md)